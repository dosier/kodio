...
import gg.kodio.core.security.AudioPermissionDeniedException
import gg.kodio.core.security.AudioPermissionManager
...
object AndroidAudioSystem : SystemAudioSystemImpl() {
    ...
    private val _permissionState = MutableStateFlow(AudioPermissionManager.State.Unknown)
        ...
        when (requestCode) {
            REQUEST_PERMISSION_RECORD_AUDIO -> {
                _permissionState.value = when (grantResults.getOrNull(0)) {
                    PackageManager.PERMISSION_GRANTED -> AudioPermissionManager.State.Granted
                    PackageManager.PERMISSION_DENIED -> AudioPermissionManager.State.Denied
    ...
    private suspend fun <T> withMicrophonePermission(block: () -> T): T {
        ...
        return if (!hasMicrophonePermission(context)) {
            ...
            _permissionState.value = AudioPermissionManager.State.Requesting
            coroutineScope {
                launch {
                    activity.requestPermission(RECORD_AUDIO, REQUEST_PERMISSION_RECORD_AUDIO)
                }
                launch {
                    permissionState.first { it != AudioPermissionManager.State.Requesting }
                }
            }
    ...
    private fun requireContext(): Context =
        requireNotNull(if (this::context.isInitialized) context.get() else null) {
            "AudioSystem not initialized. Call gg.kodio.core.AndroidAudioSystem.setApplicationContext(context) first."
        }

    private fun requireActivity(): Activity =
        requireNotNull(if (this::activity.isInitialized) activity.get() else null) {
            "AudioSystem not initialized. Call gg.kodio.core.AndroidAudioSystem.setMicrophonePermissionRequestActivity(activity) first."
        }
}
