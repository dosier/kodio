...
abstract class BaseAudioRecordingSession : AudioRecordingSession {
    ...
    override suspend fun start() {
        if (state.value is AudioRecordingState.Recording) return
        ...
        try {
            ...
            recordingJob = scope.launch {
                coroutineScope {
                    // ensures that startRecording is canceled if the job is
                    ...
                    platformFlow.collect { audioChunk ->
                        hotSource.emit(audioChunk)
                    }
                }
            }
        } catch (e: Exception) {
            _state.value = AudioRecordingState.Error(e)
            cleanupRecording()
        }
    }
    ...
    override fun stop() {
        ...
        try {
            ...
            if (recordedChunks != null && format != null) {
                ...
                _state.value = AudioRecordingState.Stopped
            } else {
                _state.value = AudioRecordingState.Stopped // Or an error state if appropriate
            }
        } finally {
